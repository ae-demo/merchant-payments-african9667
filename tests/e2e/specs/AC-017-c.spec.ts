...
//
// See AC-017-a: no actor can raise a dispute in this deployment, so there is
// never one to resolve. This asserts one exists before attempting the
// resolution, which fails honestly at that point.
import { test, expect } from "@playwright/test";
import { uiSignIn, accessToken } from "../lib/auth";
import { target } from "../lib/targets";
import { api, authHeaders } from "../lib/fixtures";

test("AC-017-c: a Platform Admin can resolve a dispute in favor of the customer", async ({ page, request }) => {
  await uiSignIn(
    page,
    target("admin-webapp"),
  ...
  const resolveRes = await request.post(`${api()}/disputes/${data[0].id}/resolve`, {
    ...
    data: { outcome: "resolved-customer" },
  });
  expect(resolveRes.status(), await resolveRes.text()).toBe(200);
  expect((await resolveRes.json()).status).toBe("resolved-customer");
});
